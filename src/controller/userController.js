...
const login = async (req,res) => {
    ...
    if(!user) {
        return res.status(400).send('The user not found on provided email');
    }
    ...
    if(user && bcrypt.compareSync(req.body.password, user.password)) {
        ...
        res.status(200).send({user: user.email , token: token}) //usually token save in localStorage
    } else {
       res.status(400).send('Type again with correct password');
    }
...
const user = await UserSchema.findOne(data) 
 if (!user) res.status(400).send("This email is not registered")
...
const verifyOtp = async(req, res, next)=>{
    ...
    try{
        ...
        user.confirmPassword = bcrypt.hashSync(req.body.confirmPassword)       
         ...
         res.status(400).send(userVerifiedOtp)       
